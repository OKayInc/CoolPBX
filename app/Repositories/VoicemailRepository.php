...
class VoicemailRepository
{
    ...
    {
        $errors = [];
        $minLength = Setting::getSetting('voicemail', 'password_min_length', name: 'numeric');

        if (strlen($password) < $minLength) {
            $errors[] = "Password must be at least {$minLength} digits";
        }

        if (!ctype_digit($password)) {
            $errors[] = "Password must be numeric only";
        }

        if (preg_match('/(\d)\1{2}/', $password)) {
            $errors[] = "Password cannot contain repeating digits";
        }
        ...
        foreach ($sequences as $sequence) {
            if (strpos($password, $sequence) !== false) {
                $errors[] = "Password cannot contain sequential digits";
                break;
            }
        }
