...
class CallBlockSeeder extends Seeder
{
    ...
    {
        $conferenceControlCount = ConferenceControl::all()->count();

        if ($conferenceControlCount == 0)
        {
            $controls = config('coolpbx.conference_controls.controls');

            foreach ($controls as $control)
            {
                $newControl = ConferenceControl::create($control);
                foreach ($control['details'] as $controlDetails)
                {
                    $controlDetails['conference_control_uuid'] = $newControl->conference_control_uuid;
                    $newDetail = ConferenceControlDetail::create($controlDetails);
                }
            }
        }
